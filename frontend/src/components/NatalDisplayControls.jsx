    ...
    <div className="filter-control-panel">
      ...
      <div style={{
        ...
        {Object.entries(lineToggles)
          .filter(([key]) => lineLabels[key]) // Only render if label exists
          .map(([key, value]) => (
            <label
              key={key}
      ...
      <div style={{ borderTop: '1px solid #555', paddingTop: '0.5rem' }}>
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.3rem' }}>
              <button
                onClick={() => setLineToggles(prev => ({ ...prev, ic_mc: !prev.ic_mc }))}
                className={`control-button ${lineToggles.ic_mc ? 'success' : 'danger'}`}
                style={{ fontSize: '10px', padding: '0.2rem 0.4rem' }}
              >
                IC/MC
              </button>
              <button
                onClick={() => setLineToggles(prev => ({ ...prev, ac_dc: !prev.ac_dc }))}
                className={`control-button ${lineToggles.ac_dc ? 'success' : 'danger'}`}
                style={{ fontSize: '10px', padding: '0.2rem 0.4rem' }}
              >
                AC/DC
              </button>
...
export default NatalDisplayControls;
